...
import { 
    ...
    Link as LinkIcon, CloudUpload, RefreshCw, GripHorizontal // ✨ 여기에 GripHorizontal 추가!  
  } from "lucide-react";
...
// ✨ FINAL: 다중 카드 지원 + 오토 리사이징 + 드래그 앤 드롭 완벽 호환 메모장
const RecordsMemoEditor = ({ patient, store }: { patient: any, store: any }) => {
    const [cards, setCards] = React.useState<{id: string, title: string, content: string}[]>([]);
    const editorRefs = React.useRef<Record<string, HTMLDivElement>>({});
    ...
    const pendingSaveRef = React.useRef<{ patientId: string, cards: any[] } | null>(null);
    ...
    React.useEffect(() => {
        ...
        // 🚀 마이그레이션: 기존 통합 메모(globalMemo)가 있으면 '전체' 카드로 자동 복원
        if (patient.memoCards && patient.memoCards.length > 0) {
            setCards(patient.memoCards);
        } else if (patient.globalMemo) {
            setCards([{ id: `card-${Date.now()}`, title: '전체', content: patient.globalMemo }]);
        } else {
            setCards([{ id: `card-${Date.now()}`, title: '전체', content: '' }]);
        }
        
        return () => { isMountedRef.current = false; };
    }, [patient?.id, patient?.memoCards, patient?.globalMemo]);

    React.useEffect(() => {
        cards.forEach(card => {
            if (editorRefs.current[card.id] && editorRefs.current[card.id].innerHTML !== card.content) {
                editorRefs.current[card.id].innerHTML = card.content || "";
            }
        });
    }, [cards]);
    ...
    // ✨ 현재 화면(DOM)에 적힌 최신 텍스트를 State로 완벽 동기화하는 함수
    const syncDOMToState = () => {
        return cards.map(c => {
            const el = editorRefs.current[c.id];
            return { ...c, content: el ? el.innerHTML : c.content };
        });
    };

    const enqueueSave = () => {
        const updatedCards = syncDOMToState();
        // ✨ NEW: 입력한 글씨가 새로고침 시 날아가지 않도록 React 상태(State)에 즉시 확정 저장!
        setCards(updatedCards);
        pendingSaveRef.current = { patientId: patient.id, cards: updatedCards };
        processQueue();
    };
    ...
    const saveSelection = (id: string) => {
        ...
        if (sel && sel.rangeCount > 0 && editorRefs.current[id]?.contains(sel.anchorNode)) {
            savedRangesRef.current[id] = sel.getRangeAt(0).cloneRange();
        }
    };
    ...
    const updateToolbarState = (id: string) => {
        if (!editorRefs.current[id] || isComposingRef.current) return;
        const sel = window.getSelection();
        if (sel && sel.rangeCount > 0 && editorRefs.current[id].contains(sel.anchorNode)) {
            setIsBold(document.queryCommandState('bold'));
            setIsStrikethrough(document.queryCommandState('strikeThrough'));
            const currentColor = document.queryCommandValue('foreColor');
            if (currentColor) setActiveColor(rgbToHex(currentColor));
        }
    };

    const executeCommand = (command: string, value: string | undefined = undefined) => {
        if (!focusedCardId || isComposingRef.current) return;
        const el = editorRefs.current[focusedCardId];
        if (el && document.activeElement !== el) el.focus();
        restoreSelection(focusedCardId);
        
        document.execCommand('styleWithCSS', false, 'true');
        document.execCommand(command, false, value);
        
        saveSelection(focusedCardId);
    ...
    const addNewCard = (title: string) => {
        const updatedCards = syncDOMToState();
        const newCards = [...updatedCards, { id: `card-${Date.now()}`, title, content: '' }];
        setCards(newCards);
        store.updatePatientMemoCards(patient.id, newCards);
    }

    const deleteCard = (id: string) => {
        if(!confirm("이 메모 카드를 삭제하시겠습니까?")) return;
        const updatedCards = syncDOMToState();
        const newCards = updatedCards.filter(c => c.id !== id);
        setCards(newCards);
        store.updatePatientMemoCards(patient.id, newCards);
    }
    ...
    // ✨ NEW: 카드 드래그 시 상/하단 가장자리에 닿으면 스크롤을 자동으로 내려주는 엔진
    const cardsContainerRef = React.useRef<HTMLDivElement>(null);
    const handleContainerDragOver = (e: React.DragEvent) => {
        e.preventDefault();
        const container = cardsContainerRef.current;
        if (!container || draggedIdx === null) return;
        
        const { top, bottom } = container.getBoundingClientRect();
        const threshold = 60; // 스크롤이 발동하는 가장자리 감지 범위 (픽셀)
        const speed = 15; // 스크롤 속도
        
        if (e.clientY - top < threshold) {
            container.scrollTop -= speed; // 위로 이동
        } else if (bottom - e.clientY < threshold) {
            container.scrollTop += speed; // 아래로 이동
        }
    };
    ...
    const handleDrop = (e: React.DragEvent, idx: number) => {
        ...
        if (draggedIdx !== null && draggedIdx !== idx) {
            const updatedCards = syncDOMToState();
            const [movedItem] = updatedCards.splice(draggedIdx, 1);
            updatedCards.splice(idx, 0, movedItem);
            setCards(updatedCards);
    ...
    return (
        <div className="flex flex-col border border-slate-200 rounded-lg shadow-sm bg-[#f8fafc] overflow-hidden w-full h-full min-h-[300px] flex-1">
              <style>{`.memo-editor:empty:before { content: attr(data-placeholder); color: #94a3b8; font-style: italic; pointer-events: none; }`}</style>

            {/* 1. 툴바 영역 (스크롤을 내려도 항상 최상단에 고정됨) */}
            <div 
                className="sticky top-0 z-50 flex items-center gap-2 p-2.5 bg-white/95 backdrop-blur border-b border-slate-200 shrink-0 shadow-[inset_0_-1px_2px_rgba(0,0,0,0.02)]"
                onMouseDown={(e) => {
...
<div className="flex items-center gap-1.5 shrink-0">
                    <div className="flex flex-col items-center justify-center gap-1 shrink-0">
                        {/* ✨ 가로폭을 w-8에서 w-5로 2/3 다이어트! (세로 h-4는 유지하여 클릭 영역 방어) */}
                        <button 
                            onClick={() => executeCommand('bold')} 
                            onMouseDown={(e) => e.preventDefault()} 
                            className={`w-5 h-5 text-[10px] leading-none flex items-center justify-center rounded border border-slate-200 transition-all ${isBold ? 'bg-blue-100 text-blue-700 border-blue-300 font-extrabold shadow-sm' : 'bg-white text-slate-600 hover:bg-slate-100 font-bold'}`} 
                            title="진하게 (Bold)"
                        >
                            B
                        </button>
                        <button 
                            ...
                            onMouseDown={(e) => e.preventDefault()} 
                            className={`w-5 h-5 text-[10px] leading-none flex items-center justify-center rounded border border-slate-200 transition-all ${isStrikethrough ? 'bg-blue-100 text-blue-700 border-blue-300 font-extrabold shadow-sm' : 'bg-white text-slate-600 hover:bg-slate-100 font-bold'}`} 
                ...
                <div className="flex items-center gap-1.5 pl-1 flex-1 shrink-0">
                    {/* ✨ Color 텍스트 삭제 완료 */}
...
>
    ...
    {patient.stages?.filter((s: any) => !s.isDeleted).map((s: any) => (
        <option key={s.id} value={s.name}>{s.name}</option>
                    ...
                    <button
                        onClick={enqueueSave}
                        className={`save-btn flex items-center justify-center h-9 px-2.5 rounded text-[12px] font-black transition-all border-[1.5px] shadow-[0_2px_8px_rgba(0,0,0,0.08)] active:scale-[0.96] active:shadow-none shrink-0 whitespace-nowrap ${
                            saveState === 'saving' 
                                ? 'bg-blue-50 border-blue-300 text-blue-600' 
                                : saveState === 'error'
                                ? 'bg-red-50 border-red-300 text-red-600'
                                : 'bg-[#f8fafc] border-green-400 text-green-600 hover:bg-green-50'
                        }`}
                        title="단축키: Ctrl + S (모든 카드 동시 저장)"
                    >
                        {saveState === 'saving' ? "저장 중" : saveState === 'error' ? "저장 실패" : "메모 저장"}
                    </button>
                </div>                                                             
            </div>

{/* 2. 카드 렌더링 영역 (전체 스크롤 컨테이너) - ✨ pt-2.5 pb-5로 상단 여백만 절반으로 줄이고 하단 스크롤 공간은 보존 */}
<div 
                ref={cardsContainerRef} 
                onDragOver={handleContainerDragOver}
                className="flex-1 px-1 pt-2.5 pb-5 overflow-y-auto custom-scrollbar flex flex-col gap-3 bg-[#f8fafc]"
            >                     
                {cards.map((card, idx) => (
                    <div 
                        key={card.id} 
                    ...
                    >
                        {/* 🔵 드롭 인디케이터 (어디로 떨어질지 파란 선으로 표시) */}
                        {dragOverIdx === idx && draggedIdx !== idx && <div className="absolute -top-3 left-0 w-full h-1 bg-blue-400 rounded-full z-20 pointer-events-none" />}
...
>
    ...
    ><X className="w-3 h-3"/></button>
                        ...
                        <div 
                            className={cn(
                                "border rounded-b-xl rounded-tr-xl bg-white p-4 min-h-[120px] z-0 relative transition-all duration-300",
                                focusedCardId === card.id 
                                    ? "border-blue-400 ring-[1px] ring-blue-400 shadow-[0_4px_15px_rgba(96,165,250,0.2)]" 
                                    : "border-[#93c5fd] shadow-sm"
                            )}
                        >                                                                                            
                            <div 
                                ref={(el) => { if(el) editorRefs.current[card.id] = el; }} 
                                contentEditable 
                                onFocus={() => setFocusedCardId(card.id)}
                                onKeyDown={handleKeyDown} 
                                onCompositionStart={() => { isComposingRef.current = true; }}
                                onCompositionEnd={() => { isComposingRef.current = false; saveSelection(card.id); updateToolbarState(card.id); }}
                                onMouseUp={() => { saveSelection(card.id); updateToolbarState(card.id); }} 
                                onKeyUp={() => { saveSelection(card.id); updateToolbarState(card.id); }} 
                                data-placeholder="환자 특이사항 및 메모를 입력하세요..." 
                                className="memo-editor w-full h-full text-[15px] text-slate-700 outline-none leading-relaxed break-words whitespace-pre-wrap" 
                            />            
                        </div>
                    </div>
                ))}
...
const processMemo = (memoStr: string, targetTeeth: number[]) => {
    ...
    // ✨ NEW: 온갖 종류의 작대기(-, –, —, ~) 및 주변 띄어쓰기까지 완벽하게 대응하는 초정밀 만능 정규식!
    const match = memoStr.match(/\(?(\d+)(?:단계|段階)\s*[-~–—―]\s*(끝|終了|End|\d+)(?:단계|段階)?\)?\s*:\s*(.*)/i);
    if (match) {        
...
{/* 부가력 룰 영역 (1열 세로 배치, 편집 폼, 체크 토글) - ✨ 족쇄 해제! 룰이 0개여도 항상 표시됨 */}
